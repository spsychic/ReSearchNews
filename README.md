...
- 상단에는 `오늘의 핵심 결론 3줄`이 자동 생성됩니다. 결론은 뉴스-시장 교차검증, 시장 강도, 금·환율·06시 예측 검증 결과에서만 압축합니다.
- RSS 검색어는 `when:1d`, `when:2d`, `when:7d` 조건을 붙여 오래된 기사보다 당일성 자료를 우선 수집합니다.
- `마감`, `개장`, `장중`, `속보`, `환율`, `금리`, `나스닥`, `코스피`, `외국인` 같은 시장성 제목을 우선 정렬합니다.
- `무엇인가`, `뜻`, `개념`, `기초`, `용어`, `하는 법`처럼 당일 판단보다 설명형 콘텐츠에 가까운 제목은 수집 우선순위에서 제외합니다.
- 기사 제목의 방향성은 `호재`, `완만한 호재`, `중립`, `완만한 악재`, `악재`로 분류합니다.
- `위험선호`, `위험회피`, `금리부담`, `환율안정`, `환율부담`, `부동산회복`, `부동산부담` 태그를 붙여 시장 수치와 대조합니다.
